    ...
    def CCHMImaging(self, ScatteredField, zPositions, refShifts):
        ...
        referenceWaves = tf.exp(tf.complex(tf.cast(0., tf.float32), 2 * np.pi *
                       (tf.reshape(self.KtIllum[:, 0], [-1, 1, 1]) *
                        (tf.reshape(self.realxx, [1,  self.gridSize[1],
                                                  self.gridSize[2]]) -
                         tf.reshape(refShifts[:, 0], [1, -1, 1])) +
                        tf.reshape(self.KtIllum[:, 1], [-1, 1, 1]) *
                        (tf.reshape(self.realyy, [1, self.gridSize[1],
                                                  self.gridSize[2]]) -
                         tf.reshape(refShifts[:, 1], [1, -1, 1])))))
        ...
        for zPos in zPositions:
            ...
            # reference waves
            zStack = zStack.write(j, tf.reduce_sum(self.apodizationCCHMNotNull * \
