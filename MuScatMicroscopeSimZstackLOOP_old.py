...
class MuScatMicroscopeSim(tf.keras.Model):
    ...
    def CCHMImaging(self, ScatteredField, zPositions, refShifts):
        ...
        FiltScatteredField = self.FiltByObjectivePupil(ScatteredField)
        k = 0
        shiftedStack = tf.TensorArray(tf.complex64, size=refShifts.get_shape().as_list()[0])
        for refShift in refShifts: 
            referenceWaves = tf.exp(tf.complex(0., 2 * np.pi * (
                        tf.reshape(self.KtIllum[:, 0], [-1, 1, 1]) * \
                            (tf.reshape(self.realxx, [1, self.gridSize[1], self.gridSize[2]]) - \
                             refShift[0])
                                + tf.reshape(self.KtIllum[:, 1], [-1, 1, 1]) * \
                                    (tf.reshape(self.realyy, [1, self.gridSize[1], self.gridSize[2]]) - \
                                     refShift[1]))))
            ...
            for zPos in zPositions:
                ...
                propagator = tf.signal.ifftshift(tf.exp(tf.complex(
                    tf.cast(0., tf.float32), 2 * np.pi * tf.reshape(
                        self.KzzM, [1, self.gridSize[1], self.gridSize[2]]) * (
                        -(self.gridSize[0] * self.dz - zPos)))))
                propagatedFields = tf.signal.ifft2d(
                    tf.signal.fft2d(propagatedIllum) * propagator)                     
                zStack = zStack.write(j, tf.reduce_sum(propagatedFields * tf.math.conj(referenceWaves),
                             0) / self.planeWavesNum)
                j += 1 
            shiftedStack = shiftedStack.write(k, zStack.stack())    
            k += 1
        return shiftedStack.stack()
